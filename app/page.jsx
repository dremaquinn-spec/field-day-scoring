"use client";"use Relay","Pancake Relay","Hippity Hop Relay","Hula Hoop Bean Bag","Tug of War – Girls","Tug of War – Boys"],
    ...
    <div style={{ padding: 20 }}>
      ...
      <div>
        {TEACHERS_BY_GRADE[grade].map(t => (
          <button key={t} onClick={() => placements.length < 3 && setPlacements([...placements, t])}>
            {t}
          </button>
        ))}
      </div>
      ...
      {/* Advance button */}
      {placements.length === 3 && (
        <button onClick={() => { setPlacements([]); setEventIndex(e => e+1); }}>
          ✅ Save & Next Event
        </button>
      )}
    </div>
